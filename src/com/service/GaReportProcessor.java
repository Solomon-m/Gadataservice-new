...
public class GaReportProcessor {
	...
	//solomon change for Agent Action Report
	HashMap outerhminmain_AAR = new HashMap();
	HashMap ohmforremovefault_AAR = new HashMap();
	ArrayList finalArraylist_AAR =  new ArrayList();
    ...
    public ArrayList AgentActionReport(ArrayList<ArrayList<?>> jsonData) throws JsonGenerationException, JsonMappingException, IOException
    {
    	...
    	Gson gson1 = new Gson();
//    	String testjson = gson1.toJson(jsonData);
//    	System.out.println("Data is ::"+testjson);
    	/*------------------------------------------------------*/
    	 HashMap innerhminmain = new HashMap();
		 for(int i=0; i<jsonData.size(); i++){
        	  HashMap innertemp = new HashMap();
        	  
        	  innertemp = V2CorrectionforAgentAction.convertArraytoMap((ArrayList) jsonData.get(i),i);
        	  HashMap temp2 = new HashMap(innertemp);
        	 // innerhminmain.putAll(innertemp);
        	  innerhminmain.put(i,innertemp);			        	      
             
          }
		  outerhminmain_AAR.putAll(innerhminmain);
		  Gson gjson = new Gson();
		  /*----------------------------------------------------------*/
		  // JsonObject myjson = new JsonObject(outerhminmain);
	       // System.out.println(myjson);
		  HashMap ihmforremovefault = new HashMap();
	        for(int j=0; j<outerhminmain_AAR.size(); j++){
	        	//System.out.println("Checking ::**::"+outerhminmain.get(j));
	        	HashMap changedmap = V2CorrectionforAgentAction.removefaultfordata((HashMap) outerhminmain_AAR.get(j),j);
	        	ihmforremovefault.put(j, changedmap);
	        }
	        ohmforremovefault_AAR.putAll(ihmforremovefault);
	      //  System.out.println("After Change::"+ohmforremovefault);
	       // JSONObject myjson1 = new JSONObject(ohmforremovefault_AAR);
	        ...
	        ArrayList finalinArlist =  new ArrayList();
	         for(int k=0;k<ohmforremovefault_AAR.size(); k++){
	        	
	        	 ArrayList chagedlist = convertHmaptoArrayList((HashMap) ohmforremovefault_AAR.get(k));
	        	 //finalinArlist.add(chagedlist);
	        	 finalArraylist_AAR.add(chagedlist);
	         }
	         String testjson = gson1.toJson(finalArraylist_AAR);
	         System.out.println("FinalArrayList length is::"+finalArraylist_AAR.size());
//  	     	System.out.println("Data is ::"+testjson);
    	System.out.println("*===================ends===================*");
    	 /*
    	  * Fetching unique agent email and unique actions from rawData pulled from DataStore
        **/
    	  HashSet<String> uniqueAgentMap=getUniqueDimensionFromGAData(finalArraylist_AAR,2);
    	  HashSet<String> uniqueActionSet=getUniqueDimensionFromGAData(finalArraylist_AAR,1);
		...
		for(String agentInitial:uniqueAgentMap)
		{
			...
			ArrayList<ArrayList<?>> agentDetails=filterDataByDimension(finalArraylist_AAR,agentInitial,2);
    ...
    /*Converts Hashmap to Arraylist. Created by solomon*/
    private ArrayList convertHmaptoArrayList(HashMap ohmforremovefault) {
	ArrayList list = new ArrayList();
	list.add(ohmforremovefault.get("one"));
	list.add(ohmforremovefault.get("two"));
	list.add(ohmforremovefault.get("three"));	
	list.add(ohmforremovefault.get("four"));
	list.add(ohmforremovefault.get("five"));
	list.add(ohmforremovefault.get("six"));
	list.add(ohmforremovefault.get("seven"));
	list.add(ohmforremovefault.get("eight"));
	return list;
}
