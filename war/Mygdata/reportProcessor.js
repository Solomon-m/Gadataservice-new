...
var callMap={};
var csvData=null;
var tabList=new Array("cc","repeat","done","obDetails","feedbackData","sbNotes","annotation","save","eventToTalk","chat");
console.log("tab List set");
...
console.debug("Inside reportProcessor Mygdata::");
function navigate(page,id)
{ 
	
	if(page=="Fetch")
	{
		worker.postMessage(new createCustomObject('sendDetails',jsonData,'Fetch',3,'false',"true"));
				setActiveTab("Fetch");
	}
	if(page=="Inbound Call"){

		worker.postMessage(new createCustomObject('sendDetails',jsonData,'us-cs-telephony',3,'false',"true"));
		setActiveTab("Inbound");
		}
	if(page=="CC"){
		worker.postMessage(new createCustomObject('sendDetails',jsonData,'CallConclusion',1,'false',"true"));
		setActiveTab("cc");
		}
	if(page=="Annotation"){
		worker.postMessage(new createCustomObject('sendDetails',jsonData,'Annotation',1,'false',"true"));
		setActiveTab("annotation");
		}

	if(page=="Repeats"){
	worker.postMessage(new createCustomObject('sendDetails',jsonData,'Repeat',1,'false',"true"));
		setActiveTab("repeat");
		}
	...
	if(page=="Send"){
		...
		worker.postMessage(new createCustomObject('sendDetails',jsonData,'Done',1,'false',"true"));
		setActiveTab("done");

		}
		if(page=="Save"){

		worker.postMessage(new createCustomObject('sendDetails',jsonData,'Save',1,'false',"true"));
		setActiveTab("save");

		}
	if(page=="Chat"){

		worker.postMessage(new createCustomObject('sendDetails',jsonData,'Chat',1,'false',"true"));
		setActiveTab("chat");

		}
	
		if(page=="AR"){
		worker.postMessage(new createCustomObject('sendDetails',jsonData,'eventToTalk',1,'false',"true"));
		setActiveTab("eventToTalk");

		}	
		if(page=="Outbound")
		{
		//window.location="getCallReport.do?page=obr&dateFrom="+document.getElementById("dateFrom").value;
		worker.postMessage(new createCustomObject('sendDetails',jsonData,'Outbound Call',1,'false','true'));
		setActiveTab("obDetails");
		}
		if(page=="CX Feedback Data")
		{
		//window.location="/getCallReport.do?page=cxrData&dateFrom="+document.getElementById("dateFrom").value;
		worker.postMessage(new createCustomObject('sendDetails',jsonData,'Data',1,'false','true'));
		setActiveTab("feedbackData");
		}
	
	if(page=="V2 Outbound Report")
	{
		
		setActiveTab("v2Report");
		//window.location="/getC3OutboundReport.do?page=v2Report&dateFrom="+document.getElementById("dateFrom").value;
	}
	if(page=="SB Notes")
	{
		
		worker.postMessage(new createCustomObject('sendDetails',jsonData,'SB - Notes',1,'false','true'));
		setActiveTab("sbNotes");
...
try{
	...
	if(tabList[i]!=activeTab)
		document.getElementById(tabList[i]).parentElement.className="";
	else
	document.getElementById(activeTab).parentElement.className="active";
	}
	}
	catch(e)
	{
	console.log("looks like an invalid page,..."+e);
	}
...
console.log(index);	  dataset=getUniqueDimensionFromGAData(filteredData,index);
console.log(dataset);
  ...
  function DownloadJSON2CSV(objArray)
  {
      ...
      {
          window.open('data:text/tsv;charset=utf-8,' + escape(str));
      }
      else
      {
          var popup = window.open('','tsv','');
